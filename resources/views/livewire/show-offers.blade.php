<div>
    @forelse ($offers as $offer)
        <div wire:key="{{ $offer->id }}"
            ...
            <div class="flex gap-5 h-fit pt-6 flex-col md:flex-row md:gap-3">
                ...
                <button wire:click="$dispatch('confirm-alert',{{ $offer->id }})"
    ...
    <div x-data="{ loading: false, hasMore: @entangle('hasMore') }" class="flex flex-col items-center justify-center mt-5">
        <div x-cloak x-show="loading" x-transition>
        ...
        <button x-show="hasMore && !loading"
            x-on:click="
            let timeout = setTimeout(() => { loading = true }, 1000);
            $wire.loadMore().then(() => {
                clearTimeout(timeout);
                loading = false;
            });
        "
            class="mt-2 text-sm text-blue-400 hover:text-blue-600">{{ __('Load More...') }}</button>
    </div>
